...
sudo certbot --nginx -d armandojosegl.turnos.do --agree-tos --no-eff-email --redirect --staple-ocsp -m ajgl0001@ce.pucmm.edu.do
...
sudo cat /etc/letsencrypt/live/armandojosegl.turnos.do/fullchain.pem \
    /etc/letsencrypt/live/armandojosegl.turnos.do/privkey.pem \
    | sudo tee /etc/letsencrypt/live/armandojosegl.turnos.do/haproxy.pem
